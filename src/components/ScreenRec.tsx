...
import useScreenRecorder from '@/components/useScreenRecorder';
...
const ScreenRecordingComponent = (props: CameraProps) => {
  ...
  const [, setVideoPreviewUrl] = useState('');
  const { mediaBlobUrls, startRecording, stopRecording } = useScreenRecorder();

  const latestBlobUrl = mediaBlobUrls[mediaBlobUrls.length - 1];
  ...
  return (
    <>
      {previewVedio ? (
        ...
        <div className="relative h-screen w-screen overflow-hidden bg-gray-600">
          {recordedChunks.length > 0 ? (
            ...
            <div className="min-w-screen max-h-screen min-h-screen">
              <Webcam
                audio={false}
                ref={webcamRef}
                className="absolute bottom-10 right-14 h-40 w-40 rounded-full object-cover"
